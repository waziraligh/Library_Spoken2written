# -*- coding: utf-8 -*-
"""
Created on Sat May 29 13:56:47 2021
        ...
        while i < len(words_of_inp_para):
            ...
            if i+1 != len(words_of_inp_para):
                ...
                # When we encounter triple A or triple B
                    ...
                    i+=2
                # When we encounter words in between twenty one and ninety nine
                elif (word.lower() + " " + next_word.lower() in self.rules["General"].keys()):
                    self.out_para += " " + first_char + str(self.rules["General"][word.lower()+" "+next_word.lower()]) + last_char_next
                    i+=2
                # When we encounter words like A M or P M or C M or H T
                elif (word.upper() + " " + next_word.upper() in self.rules["General"].keys()) and (word.upper() + " " + next_word.upper() != "M L"):
                    self.out_para += " " + first_char + self.rules["General"][word.upper()+" "+next_word.upper()] + last_char_next
                    i+=2
                # Handling the case of M L
                elif  (word.upper() + " " + next_word.upper() == "M L"):
                    self.out_para += first_char + self.rules["General"][word.upper()+" "+next_word.upper()] + last_char_next
                    i+=2
                # When we encounter words like two, three, four
                elif (word.lower() in self.rules["General"].keys()) and (next_word.lower() not in ["one","two","three","four","five","six","seven","eight","nine"]):
                    self.out_para += " " + first_char + str(self.rules["General"][word.lower()]) + last_char
                    i+=1
                else:
...
def predict():
    ...
    spoken_english = to_predict_list['Spoken English Paragraph']
    conv_obj = conversion()
    written_english = conv_obj.convert(spoken_english)
    return jsonify({'Written English Paragraph': written_english})
